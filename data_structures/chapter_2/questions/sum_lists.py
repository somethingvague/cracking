...
def _sum_lists(first, second, power, carry):
    ...
    portion = (value % 10) * (10 ** power)

    power += 1
    result = Node(portion)
    result.next = _sum_lists(first.next, second.next, power, value >= 10)
    return result


def sum_lists(first, second):
    return _sum_lists(first, second, 0, 0)
